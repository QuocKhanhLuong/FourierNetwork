"""
M&M Dataset Loader for 2D Training
Loads preprocessed .npy volumes (Cardiac MRI multi-vendor)
"""
...
class MnMDataset2D(Dataset):
    """
    M&M (Multi-Centre, Multi-Vendor) Cardiac MRI Dataset for 2D slice training.
    ...
    def __init__(self, npy_dir, use_memmap=True, in_channels=3, max_cache=10):
        ...
        if os.path.exists(metadata_path):
            ...
            self.class_names = meta.get('class_names', ['BG', 'LV', 'MYO', 'RV'])
        else:
            volume_info = None
            self.num_classes = 4
            self.class_names = ['BG', 'LV', 'MYO', 'RV']
        ...
        print(f"MnMDataset2D: {len(self.index_map)} slices from {len(self.vol_paths)} volumes")
